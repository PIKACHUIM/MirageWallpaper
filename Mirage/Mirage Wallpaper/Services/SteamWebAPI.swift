...
final class SteamWebAPI {
    ...
    private let minRequestInterval: TimeInterval = 0.35
    ...
    func queryFiles(
        ...
        var components = URLComponents(string: "https://steams.524228.xyz/IPublishedFileService/QueryFiles/v1/")!
    ...
    func getFileDetails(workshopIds: [String]) async throws -> [WorkshopItem] {
        ...
        let components = URLComponents(string: "https://steams.524228.xyz/ISteamRemoteStorage/GetPublishedFileDetails/v1/")!
    ...
    func isItemDownloaded(_ workshopId: String) -> Bool {
        let workshopDir = WallpaperLibrary.shared.steamWorkshopDirectory
        let itemDir = workshopDir.appending(path: workshopId)
        return FileManager.default.fileExists(atPath: itemDir.appending(path: "project.json").path)
    }
